...
from nova.virt.fs import Clients
...
class NovaService(object):
    ...
    def wait_for_server_in_specified_status(self, server, status, timeout):
        ...
        while status_of_server != status:
            ...
            if status_of_server == vm_states.ERROR.upper():
    ...
    def reboot(self, server, reboot_type=REBOOT_SOFT):
        ...
        """
        self.nova_client.servers.reboot(server, reboot_type)
...
class CinderService(object):
    ...
    def wait_for_volume_in_specified_status(self, volume_id, status, timeout):
        ...
        while status_of_volume != status:
            ...
            if status_of_volume == 'ERROR' or status_of_volume == 'error':
    ...
    def attach(self, volume, instance_uuid, mountpoint, mode='rw'):
        return self.cinder_client.volumes.attach(volume, instance_uuid, mountpoint, mode)
